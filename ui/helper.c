...
void UI_PrintString(const char *pString, uint8_t Start, uint8_t End, uint8_t Line, uint8_t Width)
{
	...
	for (i = 0; i < Length; i++)
	{
		...
		if (pString[0] == '_')
		{
			...
			{
				/* (14 X 14 , 宋体 )*/
				{0x40,0x00,0x20,0x00,0xF8,0x3F,0x07,0x00,0x10,0x08,0x0C,0x06,0x87,0x21,0x04,0x20,0xE4,0x3F,0x04,0x00,0x84,0x00,0x14,0x03,0x0C,0x0C,0x00,0x00},/*"你",0*/
				{0x88,0x20,0x78,0x11,0x0F,0x0A,0x08,0x06,0xF8,0x19,0x00,0x00,0x42,0x00,0x42,0x20,0x42,0x20,0xF2,0x3F,0x4A,0x00,0x46,0x00,0x40,0x00,0x00,0x00},/*"好",1*/
				{0x10,0x00,0x10,0x00,0xFE,0x3F,0x10,0x20,0x10,0x20,0xFF,0x23,0x10,0x22,0x10,0x22,0x10,0x22,0x10,0x22,0xFF,0x23,0x10,0x20,0x10,0x20,0x00,0x00},/*"世",2*/
				{0x00,0x01,0x1F,0x21,0x95,0x10,0x95,0x0E,0x55,0x00,0x35,0x00,0x1F,0x00,0x35,0x00,0x55,0x00,0x95,0x3E,0x95,0x00,0x1F,0x01,0x00,0x01,0x00,0x00},/*"界",3*/
			};

			memmove(gFrameBuffer[Line + 0] + ofs, &chinses[0][0], 7);
			memmove(gFrameBuffer[Line + 1] + ofs, &chinses[0][7], 7);
			memmove(gFrameBuffer[Line + 0] + ofs, &chinses[0][14], 7);
			...
			memmove(gFrameBuffer[Line + 0] + ofs, &chinses[1][14], 7);
			...
			memmove(gFrameBuffer[Line + 0] + ofs, &chinses[2][14], 7);
			memmove(gFrameBuffer[Line + 1] + ofs, &chinses[2][21], 7);
			memmove(gFrameBuffer[Line + 0] + ofs, &chinses[3][0], 7);
			memmove(gFrameBuffer[Line + 1] + ofs, &chinses[3][7], 7);
